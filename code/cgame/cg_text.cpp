...
void CG_DrawCenterString()
{
	...
	const int fontHeight = cgi_R_Font_HeightPixels(cgs.media.qhFontMedium, scale);
	...
	while (true)
	{
		...
		const int w = cgi_R_Font_StrLenPixels(linebuffer, cgs.media.qhFontMedium, scale);

		const int x = (SCREEN_WIDTH - w) / 2;

		cgi_R_Font_DrawString(x, y, linebuffer, textcolor_center, cgs.media.qhFontMedium, -1, scale);
