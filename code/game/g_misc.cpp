...
static void setCamera(gentity_t* ent)
{
	vec3_t dir;
...
static void misc_dlight_use_old(gentity_t* ent, gentity_t* other, gentity_t* activator)
{
	G_ActivateBehavior(ent, BSET_USE);
...
"target" - what to use when it dies
*/
static void misc_atst_setanim(gentity_t* self, const int bone, const int anim)
{
	if (bone < 0 || anim < 0)
	{
		return;
