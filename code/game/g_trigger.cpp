...
void Touch_Multi(gentity_t* self, gentity_t* other, trace_t* trace)
{
	...
	if (self->spawnflags & 4)
	{
		...
		{ //we have to be holding the use key in this trigger for x milliseconds before firing
			...
			{ //start the hack
				if (other->client->ps.saber[1].Active())
				{//turn off second saber
					G_Sound(other, other->client->ps.saber[1].soundOff);
				}
				else if (other->client->ps.saber[0].Active())
				{//turn off first
					G_Sound(other, other->client->ps.saber[0].soundOff);
				}
				other->client->ps.SaberDeactivate();

				other->client->isHacking = self->s.number;
				VectorCopy(other->client->ps.viewangles, other->client->hackingAngles);

				other->client->ps.hackingTime = level.time + self->bounceCount;
				other->client->ps.hackingBaseTime = self->bounceCount;
	...
	if (self->spawnflags & 8)
	{
		...
		if (other->s.number == 0 && (other->client->ps.weapon >= WP_NUM_WEAPONS || other->client->ps.weapon <= WP_NONE
			|| !playerUsableWeapons[other->client->ps.weapon]))
		{
...
void SP_trigger_multiple(gentity_t* ent)
{
	...
	G_SpawnInt("usetime", "0", &ent->bounceCount);
...
static gentity_t* asteroid_pick_random_asteroid(const gentity_t* self)
{
	int t_count = 0;
	gentity_t* t = nullptr;
...
static int asteroid_count_num_asteroids(const gentity_t* self)
{
	int count = 0;
...
static void asteroid_move_to_start2(gentity_t* self, const gentity_t* owner_trigger)
{
