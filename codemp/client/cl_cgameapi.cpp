...
trajectory_t* CGVM_GetOriginTrajectory(const int entID)
{
	if (cgvm->isLegacy)
	{
		return reinterpret_cast<trajectory_t*>(VM_Call(cgvm, CG_GET_ORIGIN_TRAJECTORY, entID));
	}
	VMSwap v(cgvm);
...
trajectory_t* CGVM_GetAngleTrajectory(const int entID)
{
	if (cgvm->isLegacy)
	{
		return reinterpret_cast<trajectory_t*>(VM_Call(cgvm, CG_GET_ANGLE_TRAJECTORY, entID));
	}
	VMSwap v(cgvm);
...
static qboolean CL_ROFF_Play(const int entID, const int roff_id, const qboolean do_translation)
{
	return theROFFSystem.Play(entID, roff_id, do_translation, qtrue);
}
