	...
	if (self->spawnflags & 4)
	{
		...
		if (self->genericValue7)
		{
			...
			{//start the hack
				...
				other->client->ps.saberHolstered = 2;  // turn saber off if its on we need hands for console
			...
			{//hack in progress
				...
				{
					G_SetAnim(other, NULL, SETANIM_TORSO, BOTH_CONSOLE1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);
				}
				else
				{
	...
	if (self->radius)
	{
		...
		if (G_PointInBounds(eye_spot, self->r.absmin, self->r.absmax))
		{
				...
				!(other->client->pers.cmd.buttons & BUTTON_ALT_ATTACK))
			{
				...
				//should really have sound events clear the hiddenDist
				other->client->hiddenDist = self->radius;
				//movedir HAS to be normalized!
...
static void Do_Strike(gentity_t* ent)
{
	trace_t local_trace;
	vec3_t strike_from;
	vec3_t strike_point;
...
static void Think_Strike(gentity_t* ent)
{
...
static void Use_Strike(gentity_t* ent, gentity_t* other, gentity_t* activator)
{
