...
void fdct32_dual_mono(float x[], float c[])
{
	...
	for (int p = 0; p < 16; p++, pp += 2, qq -= 2)
	{
		const float t1 = 0.5F * (x[pp] + x[pp + 1]);
		const float t2 = 0.5F * (x[qq] + x[qq + 1]);
		a[p] = t1 + t2;
...
void fdct8_dual_mono(float x[], float c[])
{
	...
	/* special first stage  */
	float t1 = 0.5F * (x[0] + x[1]);
	float t2 = 0.5F * (x[14] + x[15]);
	b[0] = t1 + t2;
